# Configuration in SAP Event Mesh for updating the status of SAP CPQ quote from SAP S/4HANA Cloud after Error Resolution | SAP Help Portal

**Source:** https://help.sap.com/docs/SAP_CPQ/f80fbcd4f1c74232839c30ce26886f07/24ef377362aa4cc99e52a664186bdc30.html?locale=en-US&state=PRODUCTION&version=2603
**Crawled:** 2026-04-16
...
In case the integration between CPQ and S/4HANA Cloud creates a Solution Order in S/4HANA Cloud that contains errors, the corresponding CPQ quote is set to the status “Order created with Errors”. When the errors in S/4HANA Cloud are resolved, S/4HANA Cloud raises the Event “ErrorResolved”. This event is sent to SAP Event Mesh and triggers the integration flow which updates the status of the SAP CPQ quote to “Order Placed”.

Procedure
The following steps describe how to enable the event processing in SAP Event Mesh.
Create a queue to update SAP CPQ quote in SAP Event Mesh:
...
Enter a queue name, for example, <S4HCCPQQueue>
...
Enter a Topic in the format <Namespace>/Topic name, for example, <sap/emcc8/1/ce/sap/s4/beh/businesssolutionorder/v1/BusinessSolutionOrder/ErrorResolved/v1>. You must use the namespace that was defined in SAP Event Mesh when you defined the event mesh instance.
...
Enter a queue name, for example, <S4HCCPQQueueRetry>
...
Choose the queue that is created above, for example, <S4HCCPQQueue>
