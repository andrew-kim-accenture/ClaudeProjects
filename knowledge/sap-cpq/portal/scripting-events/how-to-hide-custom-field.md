# How to Hide Custom Field | SAP Help Portal

**Source:** https://help.sap.com/docs/SAP_CPQ/7fc27540a0f14bd1a10b639117a9d4d7/6610f5575a6449d28eb8034145d34c12.html?locale=en-US&state=PRODUCTION&version=2603
**Crawled:** 2026-04-16
...
An administrator can write scripts to dynamically show and hide custom fields.

Example

Hide custom fields using "Disallow" method, for more details see Scripting.IQuoteCustomFieldCollection.Disallow(System.String[]) method.

In this example if "SPECIFY_COLOR" custom field value is "NO", custom fields "COLOR" and "RGB_COLOR" will be hidden.



if Quote.GetCustomField("SPECIFY_COLOR").Content == "NO":
    Quote.CustomFields.Disallow("COLOR", "RGB_COLOR")
else:
    Quote.CustomFields.Allow("COLOR", "RGB_COLOR")

The same can be achieved via Scripting.IQuoteCustomField class.



if Quote.GetCustomField("SPECIFY_COLOR").Content == "NO":
    Quote.GetCustomField("COLOR").Visible = False;
    Quote.GetCustomField("RGB_COLOR").Visible = False;
else:
    Quote.GetCustomField("COLOR").Visible = True;
    Quote.GetCustomField("RGB_COLOR").Visible = True;
Yes
No