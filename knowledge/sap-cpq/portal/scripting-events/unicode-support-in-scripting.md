# Unicode Support in Scripting | SAP Help Portal

**Source:** https://help.sap.com/docs/SAP_CPQ/7fc27540a0f14bd1a10b639117a9d4d7/ce8898ff31054b4d8d79bd6170840ad3.html?locale=en-US&state=PRODUCTION&version=2603
**Crawled:** 2026-04-16
...
As IronPython uses the ascii encoding format by default, whenever values are expressed as unicode, several modifications need to be added to the script. The “N” prefix needs to be added in front of a string written in unicode to display all its constituents properly in the output.

Example


testSQL = "select Unicode_Id,Unicode_Desc,Unicode_Value from TM_Unicode_Mapping where Unicode_Desc=N'右括号' "  
result = SqlHelper.GetFirst(testSQL)  
Trace.Write(result.Unicode_Id)  
Trace.Write(result.Unicode_Desc)  
Trace.Write(result.Unicode_Value) 

Note

SAP CPQ uses MS SQL Server, so for all additional information you can refer to this page.

Furthermore, paste the first two lines of the following snippet at the start of your scripts to make them support unicode:
